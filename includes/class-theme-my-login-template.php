...
class Theme_My_Login_Template extends Theme_My_Login_Abstract {
	...
	public function get_title( $action = '' ) {
		...
		if ( is_user_logged_in() && 'login' == $action && $action == $this->get_option( 'default_action' ) ) {
			...
			if ( $page_id = Theme_My_Login::get_page_id( $action ) ) {
				...
				switch ( $action ) {
					case 'register':
						$title = __( 'Registrarse', 'theme-my-login' );
						break;
					case 'lostpassword':
					case 'retrievepassword':
					case 'resetpass':
					case 'rp':
						$title = __( 'Contraseña Perdida', 'theme-my-login' );
						break;
					case 'login':
					default:
						$title = __( 'Iniciar Sesión', 'theme-my-login' );
				}
			}
		}
	...
	public static function get_action_template_message( $action = '' ) {
		switch ( $action ) {
			case 'register':
				$message = __( 'Regístrate en Carpe Vitae', 'theme-my-login' );
				break;
			case 'lostpassword':
				$message = __( 'Por favor, introduzca su nombre de usuario o la dirección de correo electrónico. Usted recibirá un enlace para crear una nueva contraseña vía e-mail
.', 'theme-my-login' );
				break;
			case 'resetpass':
				$message = __( 'Ingrese su nueva contraseña abajo
.', 'theme-my-login' );
				break;
			default:
				$message = '';
